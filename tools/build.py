...
def build_themes():
    ...
    for s, n, d in data.THEMES:
        ...
        secs = secs + content.THEME_EXTRA.get(s, [])
        secs = secs + [(f"{n} 코스별 기본 요금", lib.P(
            f"{n}는 60분·90분·120분 코스 중 선택할 수 있습니다. 시간이 길수록 더 많은 부위를 여유 있게 다룰 수 있으며, "
            f"처음이라면 전신을 고르게 받는 90분이 무난합니다. 표시 요금은 정찰가이며 코스·인원·지역에 따라 달라질 수 있어 "
            f"예약 시 정확히 안내해 드립니다.") + lib.pmenu(note=False))]
...
def build_courses():
    ...
    for s, n, d in data.COURSES:
        ...
        if s != "price":
            secs = secs + content.COURSE_EXTRA.get(s, [])
            secs = secs + [("코스별 기본 요금", lib.P(
                "코스는 60분·90분·120분 중 선택할 수 있습니다. 시간이 길수록 더 많은 부위를 여유 있게 다룰 수 있으며, "
                "처음이라면 전신을 고르게 받는 90분이 무난합니다. 표시 요금은 정찰가이며 코스·인원·지역에 따라 달라질 수 있어 "
                "예약 시 정확히 안내해 드립니다.") + lib.pmenu(note=False))]
